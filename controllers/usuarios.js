...
const deleteUserByID = async (req = request, res = response) =>{
    ...
    try{
        ...
        //generamos la consulta
        const result = await conn.query(`UPDATE Usuarios SET Active = 'N' WHERE ID = ${id}`, (error) => {if (error) throw error})
...
const addUser = async (req = request, res = response) => {
    ...
    try {
        ...
        conn = await pool.getConnection()//Realizamos la conexión
         ...
         //generamos la consulta
        const [userExist] = await conn.query(`SELECT Usuario FROM Usuarios WHERE Usuario = 
        '${Usuario}'`)
...
const signIN = async (req = request, res = response) =>{
    ...
    try{
        ...
        //generamos la consulta
        const [user] = await conn.query(`SELECT Contrasena, Active FROM Usuarios WHERE Usuario 
        ...
        const contrasenaValidad = bcryptjs.compareSync(Contrasena, user.Contrasena)

        if (!contrasenaValidad) {
            res.status(403).json({msg: "El usuario o contraseña que se ingreso no son válidos"})
            return
        }
